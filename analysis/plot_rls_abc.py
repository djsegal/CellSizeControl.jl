...
from _pubstyle import (apply_style, clean_hexbin, opaque_legend, halo, pub_audit,
                       BLUE, VERM, GREEN)
...
def main() -> None:
    ...
    halo(axA.text(0.04, 0.93, rf"ridge corr $\rho={rho:.2f}$", transform=axA.transAxes,
    ...
    n, _, _ = axB.hist(pp, bins=bins, density=True, color=BLUE, alpha=0.55,
    ...
    assert not issues, "rls_abc pub_audit: " + "; ".join(issues)
